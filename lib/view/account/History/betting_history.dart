...
import 'package:playzone/view/account/History/aviator_bet_history.dart';
...
class _BetHistoryState extends State<BetHistory> with SingleTickerProviderStateMixin {
  ...
  List<BetIconModel> betIconList = [
    BetIconModel(title: 'Lottery', image: Assets.imagesLotteryIcon,id:30),
    BetIconModel(title: 'Mini games', image: Assets.imagesCasinoIcon,id:40),
    BetIconModel(title: 'TRX', image: Assets.imagesLotteryIcon,id:50),
                    ...
                    return GestureDetector(
                      onTap: () {
                        setState(() {
                          ...
                          selectedId=betIconList[index].id!;
                          print(selectedId);
                          print('selectedId');
  ...
  Future<void> BettingHistory() async {
    ...
    final response = await http.get(Uri.parse("https://admin.play-zone.live/api/bet_history?userid=$token&game_id=30"),);
    if (kDebugMode) {
...
class BetIconModel {
  ...
  final String? image;
  final int? id;
  BetIconModel({required this.title, this.image,this.id});
}
