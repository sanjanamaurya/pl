...
import 'package:playzone/view/home/lottery/WinGo/commonbottomsheet.dart';
...
import 'package:playzone/view/home/lottery/WinGo/dummy_grid.dart';
import 'package:playzone/view/home/lottery/WinGo/imagetoastWINGO.dart';
import 'package:playzone/view/randomlist.dart';
...
class _WinGoScreenState extends State<WinGoScreen> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    Partelyrecord(1);
    Partelyrecords(1);
    BettingHistory(gameid.toString());
    super.initState();
    selectedCatIndex = 0;
  ...
  Partelyrecord(int gameid) async {
    ...
    final response = await http.get(Uri.parse("https://admin.play-zone.live/api/result?game_id=$gameid&limit=10"));
    if (kDebugMode) {
      print(jsonDecode(response.body));
      print("https://admin.play-zone.live/api/result?game_id=$gameid&limit=10");
    }
...
Text(items.length.toString())
  ...
  Future<void> BettingHistory(String gameid) async {
    print(gameid);
    print("uuuuuuu");
    UserModel user = await userProvider.getUser();
    String token = user.id.toString();

    final response = await http.get(Uri.parse("https://admin.play-zone.live/api/bet_history?userid=$token&game_id=$gameid"),);
    if (kDebugMode) {
      print(jsonDecode(response.body));
  ...
  // Future<List<BettingHistoryModel>> BettingHistory(String gameid) async {
  ...
  //       Uri.parse("https://admin.play-zone.live/api/bet_history?userid=$token&game_id=$gameid"),
  //     );
  //     if (response.statusCode == 200) {
  //       final data =  json.decode(response.body);
  //       if(data["status"]==200){
  //         print("Coin");
  //         final List<dynamic> jsonData = json.decode(response.body)['data'] as List<dynamic>;
  //         print(jsonData);
  //         print("jsonData");
  //         List<BettingHistoryModel> AcDetails = jsonData.map((DetailsData) => BettingHistoryModel.fromJson(DetailsData)).toList();
  //
  //         return AcDetails;
  //       }else{
  //         return [];
  ...
  Partelyrecords(int gameid) async {
    ...
    final response = await http.get(Uri.parse("https://admin.play-zone.live/api/result?game_id=$gameid&limit=10&offset=$offsetResult",));
    if (kDebugMode) {
      print('pankaj');
      print(jsonDecode(response.body));
      print("https://admin.play-zone.live/api/result?game_id=$gameid&limit=10&offset=$offsetResult");
    }
